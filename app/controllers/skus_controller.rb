class SkusController < ApplicationController
  ...
  def create
    ...
    @brand = Brand.find(params[:sku][:brand_id])
    @sku.brand_id = params[:sku][:brand_id]

    respond_to do |format|
      if @sku.save
        format.html { redirect_to brand_path(@brand.id), notice: 'Sku was successfully created.' }
  ...
  private
    ...
    def sku_params
      params.require(:sku).permit(:name, :description, :quantity, :color, :weight, :width, :height, :price, :material, :other_attributes, :active)
    end
end
